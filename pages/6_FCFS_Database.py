...
class DatabaseManager:
    ...
    def save_results(self, df_results: pd.DataFrame,avg_waiting: float,avg_turnaround: float):
        with sqlite3.connect(self.db_name) as conn:
            ...
            run_number = (result[0] if result else 0) + 1
            ...
            cursor.execute(
            "INSERT INTO run_index (table_name, avg_waiting, avg_turnaround) VALUES (?, ?, ?)",
            (table_name, avg_waiting, avg_turnaround)
            )
            conn.commit()
    ...
    def fetch_history(self):
        ...
        with sqlite3.connect(self.db_name) as conn:
            ...
            cursor.execute(
            "SELECT table_name, avg_waiting, avg_turnaround, timestamp "
            "FROM run_index ORDER BY run_id DESC"
            )
