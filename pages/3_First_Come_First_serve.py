...
import importlib
FCFS_db_module = importlib.import_module("pages.6_FCFS_Database")
DatabaseManager = FCFS_db_module.DatabaseManager
...
def main():
    ...
    if "simulation_results" in st.session_state and st.session_state.simulation_results is not None:
        ...
        # 1. FCFS values come directly from this page's execution outputs
        fcfs_wait = avg_wt
        fcfs_tat = avg_tat

        # 2. Simulate Shortest Job First (SJF) metrics for these exact bursts
        ...
        # 3. Simulate Round Robin (RR) metrics using your quantum variable
        # We model the context-switch distribution based on your current quantum size
        rr_waiting_times = []
        rr_current_wait = 0
